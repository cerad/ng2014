<?php
namespace Cerad\Bundle\TournAdminBundle\Controller\Schedule\Officials;

use Cerad\Bundle\TournAdminBundle\Controller\BaseController as MyBaseController;

use Symfony\Component\HttpFoundation\Request;

class ScheduleOfficialsImportController extends MyBaseController
{
    ...
    public function processModel($model)
    {
      ...
      //    $file->getMaxFilesize(),$file->getClientSize(),$file->isValid(), $file->getError());
        ...
        $importService = $this->get('cerad_tourn.schedule_officials.import_xls');
    ...
    protected function createModelForm($model)
    {
        ...
        $builder->setAction($this->generateUrl('cerad_tourn_admin_schedule_officials_import'));
        $builder->setMethod('POST');
        
        $builder->add('attachment', 'file');
        
        $builder->add('import', 'submit', array(
            'label' => 'Import Schedule Officials',
